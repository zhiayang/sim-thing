...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	// fuck you, imgui
	const bool is_cmd_down = io.KeysDown[SDLK_RGUI & ~SDLK_SCANCODE_MASK] || io.KeysDown[SDLK_LGUI & ~SDLK_SCANCODE_MASK];
	...
	if(g.ActiveId == id)
	{
		...
		else if(IsKeyPressedMap(ImGuiKey_Enter))
		{
			...
			&& !is_ctrl_down && !is_shift_down && !is_alt_down && is_editable)
		{
		...
		else if(shortcut_modifier_down && ((IsKeyPressedMap(ImGuiKey_X) && is_editable) || IsKeyPressedMap(ImGuiKey_C))
		...
		else
		{
				...
				| ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				// is that Completion needs to disable keyboard TABBING at the moment.
				...
				if(event_key != ImGuiKey_COUNT || (flags & ImGuiInputTextFlags_CallbackAlways) != 0)
				{
					...
					const int utf8_cursor_pos = ImTextCountUtf8BytesFromStr(text, text + edit_state.StbState.cursor);
					callback_data.CursorPos = utf8_cursor_pos;

					const int utf8_selection_start = ImTextCountUtf8BytesFromStr(text, text + edit_state.StbState.select_start);
					callback_data.SelectionStart = utf8_selection_start;

					const int utf8_selection_end = ImTextCountUtf8BytesFromStr(text, text + edit_state.StbState.select_end);
					...
					{
						edit_state.StbState.cursor = ImTextCountCharsFromUtf8(callback_data.Buf,
							callback_data.Buf + callback_data.CursorPos);
					}
					if(callback_data.SelectionStart != utf8_selection_start)
					{
						edit_state.StbState.select_start = ImTextCountCharsFromUtf8(callback_data.Buf,
							callback_data.Buf + callback_data.SelectionStart);
					}
					if(callback_data.SelectionEnd != utf8_selection_end)
					{
						edit_state.StbState.select_end = ImTextCountCharsFromUtf8(callback_data.Buf,
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool alpha)
{
	...
	{
		uint32_t hexv = IM_F32_TO_INT8(col[0]) | (IM_F32_TO_INT8(col[1]) << 8)
			| (IM_F32_TO_INT8(col[2]) << 16) | (IM_F32_TO_INT8(col[3]) << 24);

		ImGui::SetTooltip("Color:\n(%.2f,%.2f,%.2f,%.2f)\n#%08X", col[0], col[1], col[2], col[3], hexv);
	}
